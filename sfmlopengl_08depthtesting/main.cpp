...
int main()
{
    ...
    sf::Window window(sf::VideoMode(1024, 768, 32), ":]", sf::Style::Titlebar | sf::Style::Close, settings);
	...
	glm::vec3 lightPos(2.7f, 3.2f, 2.0f);
	...
	const sf::Vector2i windowCenter( 512, 384 );
    ...
    while (window.isOpen())
    {
        ...
        glBindVertexArray(lightVAO);
        model = glm::mat4();
        model = glm::translate(model, glm::vec3(lightPos.x * moveX, lightPos.y, lightPos.z * moveY));
